/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { IsString, IsOptional, ValidateNested, IsEnum } from "class-validator";
import { FunctionalRequirementCreateNestedManyWithoutProjectsInput } from "./FunctionalRequirementCreateNestedManyWithoutProjectsInput";
import { Type } from "class-transformer";
import { LibraryFrameworkCreateNestedManyWithoutProjectsInput } from "./LibraryFrameworkCreateNestedManyWithoutProjectsInput";
import { EnumProjectTypeField } from "./EnumProjectTypeField";

@InputType()
class ProjectCreateInput {
  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  description?: string | null;

  @ApiProperty({
    required: false,
    type: () => FunctionalRequirementCreateNestedManyWithoutProjectsInput,
  })
  @ValidateNested()
  @Type(() => FunctionalRequirementCreateNestedManyWithoutProjectsInput)
  @IsOptional()
  @Field(() => FunctionalRequirementCreateNestedManyWithoutProjectsInput, {
    nullable: true,
  })
  functionalRequirements?: FunctionalRequirementCreateNestedManyWithoutProjectsInput;

  @ApiProperty({
    required: false,
    type: () => LibraryFrameworkCreateNestedManyWithoutProjectsInput,
  })
  @ValidateNested()
  @Type(() => LibraryFrameworkCreateNestedManyWithoutProjectsInput)
  @IsOptional()
  @Field(() => LibraryFrameworkCreateNestedManyWithoutProjectsInput, {
    nullable: true,
  })
  libraryFrameworks?: LibraryFrameworkCreateNestedManyWithoutProjectsInput;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  name?: string | null;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  tools?: string | null;

  @ApiProperty({
    required: false,
    enum: EnumProjectTypeField,
  })
  @IsEnum(EnumProjectTypeField)
  @IsOptional()
  @Field(() => EnumProjectTypeField, {
    nullable: true,
  })
  typeField?: "Option1" | null;
}

export { ProjectCreateInput as ProjectCreateInput };
